...
# Реализация
## Архитектура
-- Queue и Event Loop(scheduler) должны поддерживаться в отдельных процессах; Тогда можно содать n-ое количество scheduler которые взаимодействуют с Queue через процессы; 
-- task помещается в Queue один из scheduler его подхватывает;
