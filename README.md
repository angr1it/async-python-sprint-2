...
# Реализация
## Архитектура
-- Worker: eventloop; берет задачи (job) из scheduler;
-- Scheduler: поддерживает очередь из job; 
-- Job: Job строится из последовательности task, запуская их в цикле; 
-- Task: Составляющая Job; 
Зачем нужен scheduler и event loop; Job в процессе работы может породить новую Job, которую нужно запланироавть.